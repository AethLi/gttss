package cn.aethli.gttss.randomDataBuilder;
...
public class studentDataBuilder {
    ...
    public long accountSeeds = 201510803070L;
    ...
    public void builder() {
        ...
        batch.setBatchName("2019届毕业设计");
        batch.setBatchDate((Date) new java.util.Date());
        SysUser sysUser = new SysUser();
        sysUser.setAccount(String.valueOf(accountSeeds++));
        sysUser.setUserId(UUID.randomUUID().toString());
        sysUser.setPassword(StringUtils.toMD5("111111"));
        sysUser.setPermission(0);
        sysUser.setStatus(0);
        sysUserMapper.insertWithAll(sysUser);
        Student student = new Student();
        student.setGender(new Random().nextInt(1));
        student.setGradeName("2015");
        student.setUserId(sysUser.getUserId());
        student.setName(firstName[new Random().nextInt(firstName.length)] + lastName[new Random().nextInt(lastName.length)]);
        student.setClassCode("2abf84db-58e8-11e9-bfc6-80fa5b585ee4");
        student.setBatchCode(batch.getBatchId());
        studentMapper.insertWithAll(student);
        StudentBatchGroup studentBatchGroup = new StudentBatchGroup();
        studentBatchGroup.setBatchCode(batch.getBatchId());
